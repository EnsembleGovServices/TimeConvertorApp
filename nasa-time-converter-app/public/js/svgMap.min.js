...
function svgMapWrapper(svgPanZoom) {
      ...
      Object.keys(mapPaths).forEach(
        function (countryID) {
          ...
          if (this.options.data.values[countryID]) {
            inime.createTooltipCU(countryID);
            this.setTooltipContentCU(this.getTooltipContent(countryID),countryID);
            this.showTooltipCU(countryElement,countryID);
          }
          countryElement.addEventListener(
            'mouseenter',
            function (e) {
              ...
              this.setTooltipContentCU(this.getTooltipContent(countryID),countryID);
              this.showTooltipCU(e,countryID);
              countryElement.addEventListener(
                'mousemove',
                this.tooltipMoveEvent,
                {
    ...
    svgMap.prototype.createTooltipCU = function (countryID) {
      var tooltipcode = "tooltip"+countryID;
      var tooltipcontentcode = "tooltipContent"+countryID;
      var tooltippointercode = "tooltipPointer"+countryID;
      if (this[tooltipcode]) {
        return false;
      }
      this[tooltipcode] = this.createElement(
        'div',
        'svgMap-tooltip tool-'+countryID,
        document.getElementsByClassName('single_event_wrap')[0]
      );
      this[tooltipcontentcode] = this.createElement(
        'div',
        'svgMap-tooltip-content-wrapper tool'+countryID,
        this[tooltipcode]
      );
      this[tooltippointercode] = this.createElement(
        'div',
        'svgMap-tooltip-pointer tool'+countryID,
        this[tooltipcode]
      );
    };
    ...
    svgMap.prototype.setTooltipContentCU = function (content,countryID) {
      var tooltipcode = "tooltip"+countryID;
      var tooltipcontentcode = "tooltipContent"+countryID;
      if (!this[tooltipcode]) {
        return;
      }
      this[tooltipcontentcode].innerHTML = '';
      this[tooltipcontentcode].append(content);
    };
  
    // Show the tooltip
  
    svgMap.prototype.showTooltipCU = function (e,countryID) {
      var tooltipcode = "tooltip"+countryID;
      if (!this[tooltipcode]) {
        return;
      }
      this[tooltipcode].classList.add('svgMap-active');
      this.moveTooltip(e,countryID);
    };
  
    // Hide the tooltip
  
    svgMap.prototype.hideTooltipCU = function (countryID) {
      var tooltipcode = "tooltip"+countryID;
      if (!this[tooltipcode]) {
        return;
      }
      this[tooltipcode].classList.remove('svgMap-active');
    };
  
    // Move the tooltip
  
    svgMap.prototype.moveTooltip = function (e,countryID) {
      var tooltipcode = "tooltip"+countryID;
      if (!this[tooltipcode]) {
        return;
      }
      ...
      if(!e.pageX && !e.touches){
        var react = e.getBoundingClientRect(); 
        x = react.left + ((react.right - react.left )/2);
        y= react.top + ((react.bottom - react.top )/2);
      }
      ...
      var tWidth = this[tooltipcode].offsetWidth;
      var tHeight = this[tooltipcode].offsetHeight;
  
      // Adjust pointer when reaching window sides
      var left = x - tWidth / 2;
      if (left <= offsetToWindow) {
        x = offsetToWindow + tWidth / 2;
        this[tooltippointercode].style.marginLeft = left - offsetToWindow + 'px';
      } else if (left + tWidth >= wWidth - offsetToWindow) {
        x = wWidth - offsetToWindow - tWidth / 2;
        this[tooltippointercode].style.marginLeft =
          (wWidth - offsetToWindow - e.pageX - tWidth / 2) * -1 + 'px';
      } else {
        this[tooltippointercode].style.marginLeft = '0px';
      }
  
      // Flip tooltip when reaching top window edge
      var top = y - offsetToPointer - tHeight;
      if (top <= offsetToWindow) {
        this[tooltipcode].classList.add('svgMap-tooltip-flipped');
        y += offsetToPointerFlipped;
      } else {
        this[tooltipcode].classList.remove('svgMap-tooltip-flipped');
        y -= offsetToPointer;
      }
  
      this[tooltipcode].style.left = x + 'px';
      this[tooltipcode].style.top = y + 'px';
    };
